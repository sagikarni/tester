...
export async function getUserFromEmailAndPassword(
  email: string,
  password: string
): Promise<IMongooseUser> {
  const user = await User.findOne({ email }).populate('role');
  if (!user) {
    throw new NoSuchUserError(
      `No user have registered with e-mail address ${email}`
    );
  }
...
export async function getUserFromJwtToken(
  ...
  const user = await User.findOne({
    _id: payload._id,
    password: payload.hsh,
  }).populate('role');

  if (!user) {
    // tslint:disable-next-line:max-line-length
    throw new NoSuchUserError(
      'Could not find an user for this token, the account has been deleted or the password has been changed.'
    );
  }
