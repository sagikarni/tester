...
export default class Subdomain extends Vue {
  ...
  async load() {
    const { domain, subdomain } = this.$route.params;
    const type = DomainsModule.types
      .query()
    ...
    this.categories = [
      ...
      ...CategoriesModule.category.all().map((c: any) => ({
        ...
        children: [
          ...
          ...this.subCategories
            .filter((s: any) => s.subdomain_id === c._id)
  ...
  public async beforeRouteUpdate(to, from, next) {
    ...
    this.name = undasherize(subdomain);
