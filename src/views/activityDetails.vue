    ...
    <div>
        ...
        <section class="ex-activity-details-component" v-show="drawContent" v-if="!showErrorPane">
            <v-layout row wrap class="mb-3">
                ...
                <v-flex xs9 class="text-xs-right" style="align-self: center">
                    ...
                    <div class="menu" @click="makePin(isPinned)">
                        <pin-unpin-button :isPinned="isPinned" :loading="loading"
                                          :disabled="loading"></pin-unpin-button>
                    </div>
                </v-flex>
            </v-layout>
            ...
            <div class="ex-session-info mt-3 pt-3">
                <v-btn large color="primary" class="lets_start" @click="startActivityClick">
                    {{$locale.activities.sessionsInfo.startText }}
                </v-btn>
                <session-length @sessionInfoIdChanged="changedSessionInfoId" :sessionLengthInfo="sessionsInfo"
                                :sessionSelectedItem="sessionSelectedItem"></session-length>
            </div>
            <h3 :class="[$isRTL ? 'ex-rtl' : '', 'mt-1']">{{ $locale.activities.galleryText }}</h3>
            <image-gallery :imageGalleryInfo="imageGalleryInfo"
                           :filterId="sessionBtnId"
