        ...
        <v-layout v-bind="addColumnProp">
            <v-flex sm6 md4 xs12 v-for="(thumbnail, index) in thumbnails" :key="index">
                <div @click="showSlideImages(thumbnail)">
                    <v-card flat tile>
                        <v-img :class="{'active-item': thumbnail.active, 'inactive-item': !thumbnail.active}" :src="getImagePath(thumbnail.imgSrc, getMediaTypes.Thumbnail)" :lazy-src="getImagePath(thumbnail.imgSrc, getMediaTypes.PreLoad)" aspect-ratio="1.5">
...
export default class ImageGallery extends BaseComponent {
  ...
  public chooseGalleryThumbnails(galleryInfo: any, filterId: any) {
    ...
    setTimeout(() => {
      ...
      (TimelineMax as any).to('.inactive-item', 1.5, {
